...
####################################
# ================= NEW CUSTOM FUNCTIONS (AI & LOOPS) =================
def load_custom_emotes():
    """imot.tex ফাইল থেকে ইমোট লোড করে"""
    emotes = {}
    try:
        if os.path.exists("imot.tex"):
            with open("imot.tex", "r") as f:
                for line in f:
                    if "=" in line:
        ...
        print(f"Error loading imot.tex: {e}")
    return emotes

def ask_groq_ai_smart(user_input, emote_map):
    print(f"⏳ Sending to AI: '{user_input}'")
    try:
        api_key = "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"
        
        if not emote_map:
            return False, None, "আমার মেমোরিতে কোনো ইমোট নেই।"

        keys_list = list(emote_map.keys())
        total_emotes = len(keys_list)
        available_emotes = str(keys_list)
        ...
        system_prompt = f"""
        You are a smart matching engine.
        DATABASE: {available_emotes}
        YOUR TASK:
        1. Search the DATABASE for the item that best matches the user's input.
        2. Even if the match is fuzzy or slang (e.g. "cobra" -> "mp40" or "cobradance"), find the best KEY from the list.
        3. If a match is found, set "is_emote": true.
        4. If the input is just chat (e.g. "Hello"), set "is_emote": false.
        
        RETURN JSON ONLY:
        {{"is_emote": true, "key": "matched_key_from_database", "reply": "Your message in user's language"}}
        """
        ...
        try:
            ...
            if start_index != -1 and end_index != -1:
                ...
                return result.get("is_emote"), result.get("key"), result.get("reply")
        except:
            pass
    except Exception as e:
        print(f"❌ AI Error: {e}")
...
async def custom_file_emote_loop(uids, key, iv, region):
    """৬ সেকেন্ড পর পর ইমোট দেখানোর লুপ"""
    global evo_cycle_running
    emote_dict = load_custom_emotes()
    emote_ids = list(emote_dict.values())
    
    if not emote_ids: return

    print(f"🔄 Starting loop with {len(emote_ids)} emotes...")
    while evo_cycle_running:
        for emote_id in emote_ids:
            if not evo_cycle_running: break
            for uid in uids:
                try:
                    packet = await Emote_k(int(uid), int(emote_id), key, iv, region)
                    if online_writer:
...
# ============ CUSTOM AI & FILE COMMANDS ============
                        # imot.tex ফাইল লোড করা
                        custom_emotes_map = load_custom_emotes()
                        raw_cmd = inPuTMsG.strip().lower()

                        # ১. লুপ চালু করা (all অথবা evo লিখলে)
                        if raw_cmd == "all" or raw_cmd == "evo":
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                                await asyncio.sleep(0.5)
                            ...
                            evo_cycle_task = asyncio.create_task(
                                custom_file_emote_loop([str(uid)], key, iv, region)
                            )
                            success_msg = f"[B][C][00FF00]✅ Loop Started!\n📂 Source: imot.tex\n⏰ Delay: 6 Seconds\n"
                            await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                        # ২. AI ইমোট ম্যাচিং সিস্টেম
                        elif not raw_cmd.startswith('/'): # কমান্ড না হলে AI চেক করবে
                            if "[" in raw_cmd or "]" in raw_cmd: # নিজের মেসেজ ইগনোর
                                pass
                            else:
                                print(f"🤖 Calling AI for: {raw_cmd}...")
                                loop = asyncio.get_event_loop()
                                try:
                                    is_emote, matched_key, ai_reply = await loop.run_in_executor(
                                        None, ask_groq_ai_smart, raw_cmd, custom_emotes_map
                                    )
                                    
                                    # ইমোট পাঠানো
                                    if is_emote and matched_key and matched_key in custom_emotes_map:
                                        specific_id = custom_emotes_map[matched_key]
                                        packet = await Emote_k(int(uid), int(specific_id), key, iv, region)
                                        if online_writer:
                                            online_writer.write(packet)
                                            await online_writer.drain()
                                            print(f"✅ AI matched Emote: {matched_key}")

                                    # রিপ্লাই পাঠানো
                                    if ai_reply:
                                        formatted_msg = f"[B][C][FFFFFF]🤖 {ai_reply}"
                                        await safe_send_message(response.Data.chat_type, formatted_msg, uid, chat_id, key, iv)
...
async def MaiiiinE():
    Uid , Pw = '4343836857','B283E5F7A1B408431A284029A63499CA6B4AB7D6AB45D712A3584C17BED7E715'
